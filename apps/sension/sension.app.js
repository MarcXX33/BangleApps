// Kompletter Code für sension.app.js
(function() {
  ...
  function restoreClockUi() {
    ...
    g.clear();
    Bangle.loadWidgets();
    if (typeof require === 'function' && require('widget_utils')) {
  ...
  buildPebbleUi = function() {
    ...
    let clockInfoMenuA = clock_info_mod.addInteractive(clockInfoItems, { x : 0, y: 0, w: clockInfoW, h:clockInfoH, draw : clockInfoDraw });
    let clockInfoMenuB = clock_info_mod.addInteractive(clockInfoItems, { x : w/2, y: 0, w: clockInfoW, h:clockInfoH, draw : clockInfoDraw, startWith: 1 });
    Bangle.setUI({
      mode : "clock", redraw : draw,
      remove : function() {
        if (drawTimeout) clearTimeout(drawTimeout); drawTimeout = undefined;
        clockInfoMenuA.remove(); clockInfoMenuB.remove();
        ...
        if (typeof require === 'function' && require('widget_utils')) require("widget_utils").show();
      }
    });
  };
  ...
  Bangle.on('HRM', function(hrm) {
    ...
    if (hrm.confidence > 60) {
        ...
        if (exerciseInProgress) exerciseInProgress.updateBpm();
            ...
            draw(); if (currentBpm >= 100) showHighBpmPrompt();
        }
    } else if (!exerciseInProgress && !highBpmPromptShown) draw();
  });
